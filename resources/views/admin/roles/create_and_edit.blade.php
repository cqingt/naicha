...
@section('body')
    <blockquote class="layui-elem-quote layui-text">
        ...
        <a href="{{ url('admin/roles/index') }}"class="layui-btn" style="margin-left: 30px;">返回列表</a>
    </blockquote>
    @if(isset($roles->id))
        <form class="layui-form" action="{{ url('roles.update', $roles->id) }}" method="POST">
            <input type="hidden" name="_method" value="PUT">
            @else
                <form class="layui-form" action="{{ url('admin/roles/store') }}" method="POST">
                    @endif
        {{ csrf_field() }}
