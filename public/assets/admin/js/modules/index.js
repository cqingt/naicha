...
layui.use(['element','jquery','form', 'layedit'],function(){
    ...
    element.on('nav(left-menu)', function(elem){
        ...
        addTab(elem[0].innerText, _href, elem[0].id);
...
function addTab(tabTitle,tabUrl,tabId){
    if ($(".layui-tab-title li[lay-id=" + tabId + "]").length > 0) {
        ...
        element.tabAdd('tab-switch', {
            title: tabTitle
            ,content: '<iframe src='+tabUrl+' width="100%" height="660px" style="height: 660px;" frameborder="0" scrolling="auto" onload="setIframeHeight(this)"></iframe>' // 选项卡内容，支持传入html
